...
function daylightMap(element, currentdate, offset) {
	...
	for (var x = 0; x < width; x++) {
		for (var y = 0; y < height; y++) {
			...
			var p1 = Math.cos(2*Math.PI*x/width*0.8)*Math.sin(Math.PI*y/height/7);
			var p2 = Math.sin(-2*Math.PI*x/width*0.8)*Math.sin(Math.PI*y/height/7);
			var p3 = Math.cos(Math.PI*y/height*0.8); // so many roll up
			...
			if (brightness > 0.99) {
				setPixel(imageData, x, y, 0, 0, 0, 50
					//- 3000*(brightness-0.99) this is Sun
				);
